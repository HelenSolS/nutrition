...
import {
  ...
  Pencil, MoveVertical, Clock, Images
} from 'lucide-react'
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="p-3 bg-amber-50 border border-amber-200 rounded-xl text-xs text-amber-700">
        <strong>Предпросмотр:</strong> совет будет выделен золотой карточкой — участник сразу его заметит
      </div>
    </div>
  )
}
...
function SurveyBlockEditor({ block, onChange }) {
  ...
  return (
    <div className="space-y-4">
      <div>
        <label className="label-xs">Заголовок опроса</label>
        <input className="input-field" value={block.title || ''} onChange={e => onChange({ ...block, title: e.target.value })} placeholder="Опрос дня" />
      </div>
...
// ─── Day editor panel ─────────────────────────────────────
function DayEditor({ day, marathon, onUpdateDay, onSaveDayToLib, onClose }) {
      ...
      <div className="space-y-3 flex-1">
        ...
        <div className="relative">
          <button onClick={() => setShowAddBlock(!showAddBlock)}
            className="w-full py-3.5 border-2 border-dashed border-gray-200 rounded-2xl text-sm font-medium text-gray-400 hover:border-violet-400 hover:text-violet-600 transition-all flex items-center justify-center gap-2">
            <Plus size={17} />Добавить блок
          </button>
          {showAddBlock && (
            <div className="absolute top-full left-0 right-0 mt-2 bg-white rounded-2xl card-shadow-lg border border-gray-100 p-4 z-20">
              <div className="flex justify-between mb-3">
                <p className="text-sm font-semibold text-gray-700">Тип блока</p>
                <button onClick={() => setShowAddBlock(false)} className="p-1 text-gray-400"><X size={13} /></button>
              </div>
              <div className="grid grid-cols-3 gap-2 mb-3">
                {BLOCK_TYPES.map(bt => (
                  <button key={bt.type} onClick={() => addBlock(bt.type)}
                    className={`flex flex-col items-center gap-1.5 p-3 rounded-xl hover:ring-2 hover:ring-violet-300 transition-all ${bt.color}`}>
                    {bt.icon}<span className="text-xs font-medium">{bt.label}</span>
                  </button>
                ))}
              </div>
              <button onClick={() => { setShowBlockLib(true); setShowAddBlock(false) }}
                className="w-full flex items-center justify-center gap-2 py-2.5 bg-violet-50 text-violet-700 rounded-xl text-sm font-medium hover:bg-violet-100 transition-colors">
                <BookMarked size={14} />Из библиотеки блоков
              </button>
            </div>
          )}
        </div>
      ...
      <div className="max-w-7xl mx-auto px-6 py-6 flex gap-5">
        ...
        <div className="w-52 shrink-0">
          <div className="flex items-center justify-between mb-3">
            ...
            <button onClick={() => setShowDayLib(true)} className="p-1.5 hover:bg-violet-50 rounded-lg text-gray-400 hover:text-violet-600 transition-colors" title="Библиотека дней"><FolderOpen size={14} /></button>
          </div>
        ...
        {activeDay ? (
          ...
          <div className="flex-1 flex items-center justify-center">
            <div className="text-center">
              ...
              <h3 className="font-semibold text-gray-600 mb-2">Выбери день для редактирования</h3>
              <p className="text-sm text-gray-400 mb-4">Или создай новый урок</p>
